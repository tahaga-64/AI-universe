      ...
      <section id="signals" className="relative mx-auto max-w-7xl px-6 py-24">
        <div className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="mt-3 text-4xl font-semibold tracking-[-0.05em] md:text-6xl">
              Twenty curated AI orbits
            </h2>
          </div>
          <p className="max-w-lg text-slate-400">
            Mock editorial data from leading AI organizations, structured for
            extensible feeds and future API integration.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {newsItems.map((item) => (
            <article
              key={item.id}
              className="rounded-[1.75rem] border border-white/10 bg-white/[0.035] p-5 transition hover:-translate-y-1 hover:bg-white/[0.06]"
            >
